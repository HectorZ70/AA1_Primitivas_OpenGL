...
void RenderManager::Initialize(const char* vertexShaderPath, const char* geometryShaderPath, const char* fragmentShaderPath)
{
    ...
    tintStrengthLocation = glGetUniformLocation(shaderProgram, "tintStrength");
...
void RenderManager::SetLight(const glm::mat4& modelMatrix, const glm::vec3& camPos, const glm::vec3& camForward, bool flashOn)
{
    glUseProgram(shaderProgram);
...
void RenderManager::SetDayNightUniforms(const glm::vec3& sunDir, float sunIntensity, const glm::vec3& moonDir, float moonIntensity, const glm::vec3& ambColor, float ambStrength)
{
    glUseProgram(shaderProgram);
...
void RenderManager::SetFlashlightUniforms(bool on, const glm::vec3& pos, const glm::vec3& dir, float innerCutoff, float outerCutoff, float range)
{
    glUseProgram(shaderProgram);
...
// Render Model
void RenderManager::Render(const Model& modelObj, const glm::mat4& mvpMat, const glm::mat4& modelMat, float time, int objectType, glm::vec4 tintColor, float tintStrengthValue)
{
    glUseProgram(shaderProgram);
    ...
    glUniform4fv(tintLocation, 1, glm::value_ptr(tintColor));
    glUniform1f(tintStrengthLocation, tintStrengthValue);
