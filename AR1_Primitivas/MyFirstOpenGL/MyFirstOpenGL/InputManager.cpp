...
void InputManager::Update(GLFWwindow* window, TimeManager& time)
{
    // Pause
    if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && !keyPressed[GLFW_KEY_SPACE])
    {
    ...
    // Speed down
    if (glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS && !keyPressed[GLFW_KEY_N])
    {
        time.SetTimeScale(time.GetTimeScale() * SPEED_DOWN_FACTOR);
